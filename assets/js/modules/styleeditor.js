(function (){
	...
	var canvasElement = require('./canvasElement').Element;
	var bConfig = require('./config');
	var siteBuilder = require('./builder');
    ...
    var styleeditor = {
        ...
        init: function() {
            ...
            $(this.formStyle).on('click', 'label.asInput', function () {
                ...
                document.body.onfocus = function () {
                    $(styleeditor.formStyle).find('input[name="background-image"]').trigger('change');
                    document.body.onfocus = null;
                };

            });
            $(this.formStyle).on('change', 'input[name="background-image"]', function () {

                $(this).next().animate({'width': '42%'}, 500, function(){
                    $(this).css('position', 'relative');
                    $(this).css('right', 'auto');
                });

                //set the file name as the innerText of the label
                if ( this.value ) $(this).next().text( utils.getFileNameFromInput(this) );
            ...
            for( var x = 0; x < bConfig.editableItems[theSelector].length; x++ ) {
                ...
                if( theSelector + " : " + bConfig.editableItems[theSelector][x] in bConfig.editableItemOptions) {//we've got a dropdown instead of open text input
                    ...
                    if( bConfig.editableItems[theSelector][x] === 'background-image' ) {
                        ...
                        newInput.id = 'background-image';
                        newInput.name = 'background-image';
                        newInput.style.display = 'none';

                        newStyleEl.find('input').remove();

                        newStyleEl.get(0).appendChild(newInput);

                        var newLabel = document.createElement('LABEL');
                        newLabel.setAttribute('for', 'background-image');
                        newLabel.classList.add('asInput');
                        newLabel.innerText = utils.getFileFromBackgroundImage( $(styleeditor.activeElement.element).css( bConfig.editableItems[theSelector][x] ) );


                        newStyleEl.get(0).appendChild(newLabel);
        ...
        */
        updateStyling: function() {
            ...
            //do we need to upload an image? (for background-image)
            if ( styleeditor.formStyle.querySelectorAll('input[type="file"]').length > 0 ) {

                var fd = new FormData();
                fd.append('imageFileField', styleeditor.formStyle.querySelector('input[type="file"]').files[0]);
                ...
                $.ajax({
                    ...
                    if( response.code === 1 ) {//success
                        ...
                        styleeditor.activeElement.element.style.backgroundImage = "url(" + response.response + ")";
            ...
            //do we need to upload an image? (for IMG tags)
